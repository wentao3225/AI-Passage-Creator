package com.ywt.passage.core.manager;
...
import static com.ywt.passage.constant.ArticleConstant.SSE_RECONNECT_TIME_MS;
import static com.ywt.passage.constant.ArticleConstant.SSE_TIMEOUT_MS;
...
public class SseEmitterManager {
    ...
    /**
     * 记录已提示过“Emitter 不存在”的任务，避免高频日志刷屏
     */
    private final Set<String> missingEmitterWarnedTaskSet = ConcurrentHashMap.newKeySet();
    ...
    public SseEmitter createEmitter(String taskId) {
        ...
        missingEmitterWarnedTaskSet.remove(taskId);
    ...
    public void send(String taskId, String message) {
        ...
        if (emitter == null) {
            if (missingEmitterWarnedTaskSet.add(taskId)) {
                log.warn("SSE Emitter 不存在, 将跳过后续推送, taskId={}", taskId);
            }
            return;
        }
    ...
    public void complete(String taskId) {
        SseEmitter emitter = emitterMap.get(taskId);
        if (emitter == null) {
            log.debug("SSE Emitter 不存在, taskId={}", taskId);
            return;
        }
