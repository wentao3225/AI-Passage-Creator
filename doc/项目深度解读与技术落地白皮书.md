...
本项目当前已经从“单次提交、一次性生成”的 AI 写作 Demo，演进为一个 **“多 Agent 协同创作 + 分阶段交互式确认 + 异步任务编排 + SSE 实时回传 + Agent 执行日志与统计 + 多源配图 + 并行配图生成 + 本地静态资源落盘”** 的全栈系统。
...
- 任务过程通过 SSE 实时可视化；在旧版 `ArticleAgentService` 路径（或 `aiModifyOutline` 调用）下，`@AgentExecution` 会额外把执行状态、耗时和错误信息落库，供详情页回看。

因此，这已经不是传统意义上的 CRUD 管理后台，而是一个 **“多 Agent 驱动、状态机控制、具备执行可观测性的人机协作创作系统”**。
...
- **Spring Data Redis + Spring Session Redis**（`pom.xml`、`application.yaml`）
  - Session 登录态存储与跨请求共享。
- **Spring AOP**（`pom.xml`）
  - `@AuthCheck` 与 `@AgentExecution` 两类横切逻辑的承载层，同时支撑权限校验与执行日志采集。
- **Spring AI Alibaba Agent Framework + DashScope Starter**（`pom.xml`）
  - 提供大模型调用、流式响应与 Agent 编排基础；当前 `application.yaml` 默认配置的聊天模型为 `glm-5`。
- **Hutool**（`pom.xml`）
  - 通过 `IdUtil.simpleUUID()` 生成稳定任务 ID，并提供基础工具能力。
- **Gson**（`pom.xml`）
...
- **Vue 3.5.31**（`passage-web/package.json`）
  - 页面与组件的响应式交互框架。
- **Vite 8.0.3**（`passage-web/package.json`、`vite.config.ts`）
  - 开发服务器与构建工具。
- **TypeScript 6.0.0 + vue-tsc 3.2.6**（`passage-web/package.json`）
  - 类型约束与编译期检查。
...
5. `aop` 与 `aspect` 组成横切治理层，分别处理 Agent 执行日志采集与权限拦截。
6. `agent` 是编排器与节点层，承载 StateGraph 编排、节点动作实现和并行配图执行。
7. `config` 承载线程池、静态资源映射、图片来源配置与额外信任库初始化。
8. 前端 `pages/article` 负责创作、编辑、历史、详情与执行日志展示等用户交互闭环。
...
2. `@EnableAspectJAutoProxy(exposeProxy = true)` 让 `@AuthCheck` 与 `@AgentExecution` 切面生效，并支持 `ArticleAgentService` 通过 `AopContext.currentProxy()` 触发同类方法上的 AOP 代理。
...
6. `AgentConfig` 通过 `article.agent.orchestrator.enabled` 配置“默认启用多智能体编排器、可回退旧版 `ArticleAgentService`”的运行模式。
7. `LocalUploadResourceConfig` 把本地上传目录映射到 `/uploads/**`，让图片落盘后可直接访问。
...
- `src/main/java/com/ywt/passage/agent/ArticleAgentOrchestrator.java`
- `src/main/java/com/ywt/passage/agent/config/AgentConfig.java`
- `src/main/java/com/ywt/passage/agent/agents/TitleGeneratorAgent.java`
- `src/main/java/com/ywt/passage/agent/agents/OutlineGeneratorAgent.java`
...
- 作用：异步生成标题方案，按配置路由到 `ArticleAgentOrchestrator`（默认）或 `ArticleAgentService`（回退），保存 `titleOptions`，推送 `TITLES_GENERATED`
...
- 作用：在用户确认大纲后生成正文、分析配图、并行生成图片、合成全文并完结任务

4. `ArticleAgentOrchestrator.executePhase1_GenerateTitles(...)`

- 基于 StateGraph 触发 `TitleGeneratorAgent` 节点，生成 3 到 5 个标题方案

5. `ArticleAgentOrchestrator.executePhase2_GenerateOutline(...)`

- 基于 StateGraph 触发 `OutlineGeneratorAgent` 节点，根据确认后的标题和用户补充描述流式生成大纲

6. `ArticleAgentOrchestrator.executePhase3_GenerateContent(...)`

- 依次触发 `ContentGeneratorAgent -> ImageAnalyzerAgent -> ParallelImageGenerator -> ContentMergerAgent`，完成正文、配图需求、并行配图和图文合成

7. `TitleGeneratorAgent.parseTitleOptions(...)` 与 `ImageAnalyzerAgent.parseAgent4Result(...)`

- 通过 `LlmJsonUtils` 对模型输出做“代码块剥离、JSON 片段提取、语法修复、字符串包裹 JSON 解包”等容错解析，降低格式抖动导致的任务失败

#### 代码逻辑流

当前运行时为“可切换双引擎”，默认开启多智能体编排器：

1. `article.agent.orchestrator.enabled=true`：走 `ArticleAgentOrchestrator` + StateGraph 节点编排
2. `article.agent.orchestrator.enabled=false`：回退到 `ArticleAgentService` 的传统串行实现

在默认编排器模式下，用户层面是三阶段创作，内部链路为：

1. Agent1：标题方案生成
2. Agent2：大纲流式生成
3. Agent3：正文流式生成
4. Agent4：配图需求分析
5. Agent5：按来源分组并行执行配图
6. merge：图文合成

与旧版本最大的区别是：**标题和大纲不再直接贯穿到底，而是中途停下来等待用户确认；正文与配图生成同时支持“编排器模式（默认）”和“旧版服务模式（回退）”；并且针对 LLM 返回格式不稳定引入了容错 JSON 解析。**
...
## 模块 D：Agent 执行可观测性与统计模块

#### 核心文件路径

- `src/main/java/com/ywt/passage/annotation/AgentExecution.java`
- `src/main/java/com/ywt/passage/aop/AgentExecutionAspect.java`
- `src/main/java/com/ywt/passage/agent/config/AgentConfig.java`
- `src/main/java/com/ywt/passage/core/service/ArticleAgentService.java`
- `src/main/java/com/ywt/passage/entity/AgentLog.java`
- `src/main/java/com/ywt/passage/service/AgentLogService.java`
- `src/main/java/com/ywt/passage/service/impl/AgentLogServiceImpl.java`
- `src/main/java/com/ywt/passage/model/vo/AgentExecutionStats.java`
- `src/main/java/com/ywt/passage/controller/ArticleController.java`
...
- 作用：标记需要被采集执行日志的 Agent 方法（当前主要覆盖 `ArticleAgentService` 及 `aiModifyOutline` 路径）。

2. `AgentExecutionAspect.aroundAgentExecution(...)`

- 作用：在 Agent 方法执行前后自动提取 `taskId`、输入摘要、状态、耗时、错误信息并异步持久化。

3. `AgentLogServiceImpl.getExecutionStats(taskId)`

- 作用：聚合 `agent_log` 明细，生成总耗时、总体状态、各 Agent 耗时映射和时间线数据。

4. `ArticleController.getExecutionLogs(taskId)`

- 作用：向前端暴露任务执行统计接口。

5. `ArticleDetailPage.loadExecutionLogs(taskId)`

- 作用：详情页加载文章后继续请求执行统计，并渲染可折叠时间线。

#### 代码逻辑流

1. `ArticleAgentService` 中的 `agent1GenerateTitleOptions`、`agent2GenerateOutline`、`agent3GenerateContent`、`agent4AnalyzeImageRequirements`、`agent5GenerateImages` 与 `aiModifyOutline` 带有 `@AgentExecution`，因此在“旧版服务路径”下会产生完整 `agent_log`。
2. `AgentExecutionAspect` 在进入目标方法时先构造一条 `RUNNING` 状态日志，并从 `ArticleState` 或字符串参数中提取 `taskId`。
3. 方法执行成功时记录 `SUCCESS`、`durationMs` 与输出摘要；执行异常时记录 `FAILED` 与错误信息。
4. 日志通过 `AgentLogService.saveLogAsync(...)` 异步落入 `agent_log` 表，不阻塞主生成链路。
5. 默认编排器路径（`article.agent.orchestrator.enabled=true`）主要依赖 SSE 实时日志；详情页执行日志面板消费的是持久化 `agent_log` 聚合结果，两者职责不同。
...
3. `ImageAnalyzerAgent.apply(...)`

- 作用：让 LLM 输出配图需求，并通过 `LlmJsonUtils` 做容错解析；随后基于允许的图片方式做二次约束

4. `ParallelImageGenerator.apply(...)`

- 作用：按 `imageSource` 分组并行执行图片生成，同类串行、异类并行，并逐张推送 `IMAGE_COMPLETE`

#### 代码逻辑流

1. 正文生成后，`ImageAnalyzerAgent` 先在正文中插入图片占位符，并输出每张图的 `position`、`imageSource`、`keywords` 或 `prompt`；解析阶段会做“去代码块、提取 JSON、字符串包裹解包、语法修复”等容错处理。
...
4. 在默认编排器模式下，`ParallelImageGenerator` 会按来源分组并发生成配图，提升多图场景吞吐。
5. `ImageServiceStrategy` 在启动时自动注册所有 `ImageSearchService` 实现，避免在业务层硬编码分支。
6. 图片拿到后统一写入 `src/main/resources/static/uploads/{source}/{yyyyMMdd}`；运行时降级图则会写入 `fallback/{yyyyMMdd}` 子目录。
7. 通过 `/api/uploads/**` 对外暴露访问路径，数据库中存储的是可直接访问的 URL。
...
- 详情页会自动拉取 `/article/execution-logs/{taskId}` 并展示执行时间线、总耗时与总体状态；
- 详情页支持导出 Markdown；
- 历史页展示状态、风格、阶段、配图数量；
...
补充说明：当前 `sql/init.sql` 采用“基础 `CREATE TABLE` + 后续 `ALTER TABLE` 扩展字段”的方式维护 `style`、`phase`、`titleOptions`、`userDescription`、`enabledImageMethods` 等列；同一份 SQL 还会创建 `agent_log` 表，因此新环境初始化时需要执行完整 SQL 文件，旧环境升级时则需要关注字段和日志表是否已存在。

### 实体：AgentLog

来源：`src/main/java/com/ywt/passage/entity/AgentLog.java`、`sql/init.sql`

关键字段：

- `id`：主键
- `taskId`：关联文章任务 ID
- `agentName`：执行节点名称，例如 `agent3_generate_content`
- `startTime / endTime / durationMs`：执行起止时间与耗时
- `status`：`RUNNING / SUCCESS / FAILED`
- `errorMessage`：失败原因
- `prompt`：当前实现中记录的方法签名标识
- `inputData / outputData`：裁剪后的输入输出摘要 JSON
- `createTime / updateTime`
- `isDelete`：逻辑删除

作用：

- 持久化带 `@AgentExecution` 节点的运行轨迹（当前主要覆盖旧版 `ArticleAgentService` 路径及 `aiModifyOutline` 调用）；
- 为详情页执行时间线与任务统计接口提供基础数据；
- 保留错误定位和性能观察所需的最小信息集合。
...
### 对外对象：AgentExecutionStats

来源：`src/main/java/com/ywt/passage/model/vo/AgentExecutionStats.java`

作用：

- 面向前端输出任务的执行统计视图；
- 聚合 `taskId`、`totalDurationMs`、`agentCount`、`agentDurations`、`overallStatus` 与 `logs`；
- 被 `/article/execution-logs/{taskId}` 接口直接返回，供详情页执行日志面板消费。
...
8. 阶段 3 依次完成正文、配图需求、按来源并行配图、图文合成与数据库回写。
9. 最终通过 `ALL_COMPLETE` 告知前端进入完成态。

### 链路 C：图片获取与落盘

1. `ImageAnalyzerAgent` 为正文生成配图需求清单，并对 LLM 返回做容错解析。
2. `ParallelImageGenerator` 按来源分组并发调度，内部再为每张图构造 `ImageRequest`。
...
1. 在旧版 `ArticleAgentService` 路径（或 `aiModifyOutline` 调用）中，带有 `@AgentExecution` 的方法会进入 `AgentExecutionAspect` 环绕通知。
2. 切面抽取 `taskId`、输入摘要和方法标识，创建日志对象并在方法完成后补齐状态、耗时、错误信息与输出摘要。
3. `AgentLogServiceImpl.saveLogAsync(...)` 将日志异步落库到 `agent_log`。
4. 详情页先加载 `/article/{taskId}` 获取文章主体，再调用 `/article/execution-logs/{taskId}` 拉取统计结果。
5. 前端将 `overallStatus`、总耗时和明细日志渲染为可折叠时间线；与创作页 SSE 实时日志形成“历史回看 + 实时观测”的互补。
...
sequenceDiagram
    ...
    participant AO as ArticleAgentOrchestrator(default)
    participant LG as ArticleAgentService(legacy/aiModify)
    ...
    AYS->>AO: executePhase1_GenerateTitles(state)
    AO-->>AYS: AGENT1_COMPLETE
    ...
    AYS->>AO: executePhase2_GenerateOutline(state)
    AO-->>FE: AGENT2_STREAMING(content chunks)
    ...
    opt 用户使用 AI 修改大纲
        ...
        AS->>LG: aiModifyOutline(...)
        LG-->>AS: modifiedOutline
    ...
    AYS->>AO: executePhase3_GenerateContent(state)
    AO-->>FE: AGENT3_STREAMING(content chunks)
    AO-->>FE: AGENT4_COMPLETE(imageRequirements)
    AO->>ISS: getImageAndUpload(imageSource, imageRequest)
    ISS->>FS: 保存图片到 uploads/{source}/{date}
    ISS-->>AO: 返回本地访问 URL
    AO-->>FE: IMAGE_COMPLETE(image)
    AO-->>FE: AGENT5_COMPLETE(images)
    AO-->>FE: MERGE_COMPLETE(fullContent)
...
补充说明：默认编排器主链路以 SSE 实时日志为主；`agent_log` 持久化日志主要来自旧版 `ArticleAgentService` 路径及 `aiModifyOutline` 调用。详情页会继续请求 `/article/execution-logs/{taskId}` 渲染时间线与统计概览。
...
- `src/main/resources/application.yaml`
  ...
  - `spring.ai.dashscope`：DashScope API Key 与聊天模型配置，当前默认模型为 `glm-5`
...
1. 准备依赖环境：JDK 21、Maven、Node.js `^20.19.0` 或 `>=22.12.0`、MySQL、Redis。
2. 如果要使用 Mermaid 配图，额外安装 Mermaid CLI。
3. 执行数据库初始化：导入完整的 `sql/init.sql`。
4. 当前 `sql/init.sql` 同时包含基础建表语句、后续 `ALTER TABLE` 扩展字段语句，以及 `agent_log` 表定义；如果数据库是旧版，需要确认 `style`、`userDescription`、`enabledImageMethods`、`titleOptions`、`phase` 等字段以及 `agent_log` 表已经同步，避免重复执行时报错。
...
- 主流程：
  1. 校验 `taskId` 非空
  2. 调用 `AgentLogService.getExecutionStats(taskId)` 聚合执行统计
  3. 返回总体状态、总耗时、各 Agent 耗时与明细日志
- 返回：`AgentExecutionStats`

补充说明：当前详情页会在加载文章详情后自动请求该接口。按照当前控制器实现，该接口本身只校验 `taskId` 非空，然后按 `taskId` 聚合日志统计。
补充说明：在默认编排器模式下，该接口返回的数据可能主要来自 `aiModifyOutline` 或历史旧版链路日志；实时进度应以 SSE 通道消息为准。
...
截至 2026-04-22，项目已经形成了较清晰的“分层 + 阶段状态机 + 双引擎可切换编排（默认 Orchestrator）+ 流式回传 + 条件化执行可观测性 + 多源并行配图 + 本地静态资源”架构：
...
- 可观察性落地：默认链路依赖 SSE 实时日志；在旧版 `ArticleAgentService` 路径与 `aiModifyOutline` 调用中，AOP 会记录执行耗时、状态和错误信息，详情页可回看时间线与统计；
