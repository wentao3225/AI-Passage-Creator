...
本项目当前已经从“单次提交、一次性生成”的 AI 写作 Demo，演进为一个 **“分阶段交互式创作 + 异步任务编排 + SSE 实时回传 + 多源配图 + 本地静态资源落盘”** 的全栈系统。
...
因此，这已经不是传统意义上的 CRUD 管理后台，而是一个 **“状态机驱动的人机协作创作系统”**。
...
3. `core/service` 是任务编排层，负责三阶段异步执行和 LLM 编排。
...
与旧版本最大的区别是：**标题和大纲不再直接贯穿到底，而是中途停下来等待用户确认。**
...
2. 如果模型给出的来源不在用户允许列表中，后端会强制替换为允许的方式，而不是直接照用。
3. `ImageServiceStrategy` 在启动时自动注册所有 `ImageSearchService` 实现，避免在业务层硬编码分支。
4. 图片拿到后统一写入 `src/main/resources/static/uploads/{source}/{yyyyMMdd}`。
5. 通过 `/api/uploads/**` 对外暴露访问路径，数据库中存储的是可直接访问的 URL。
6. 当真实来源失败时，系统会自动降级到 `PICSUM`，保证整条文章生成链路不中断。
...
3. `ImageServiceStrategy` 根据 `imageSource` 选择具体服务。
4. 图片服务返回 `ImageData`。
...
3. 执行数据库初始化：导入 `sql/init.sql`。
4. 如果数据库是旧版，需要确保 `style`、`userDescription`、`enabledImageMethods`、`titleOptions`、`phase` 等字段已经同步。
...
截至 2026-04-18，项目已经形成了较清晰的“分层 + 阶段状态机 + AI 编排 + 流式回传 + 多源配图 + 本地静态资源”架构：

- 分层明确：Controller 做入口，Service 做规则，Core 做编排，Image Strategy 做多源图片治理；
